...
export const getCurrentScheme = async () => {
  ...
  if (typeof window === "undefined") {
    return import("next/headers").then(({ cookies }) => {
      return cookies().has("prefers-color-scheme")
        ? cookies().get("prefers-color-scheme")?.value
        : "light";
    });
  }

  return getCookie("prefers-color-scheme", { path: "/" });
};
...
export const toggleScheme = async () => {
  ...
  setCookie("prefers-color-scheme", newScheme, {
    path: "/",
  });
